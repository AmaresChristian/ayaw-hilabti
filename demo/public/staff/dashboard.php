...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['add'])) {
        $name = $_POST['name'] ?? '';
        $stock = $_POST['stock'] ?? '';
        if (!$medicineCtrl->add($name, $stock)) {
...
<title>Staff Dashboard - Pharmacy Inventory</title>
...
<main>
  <!-- <section aria-labelledby="addMedicineTitle">
    ...
    <form method="post" enctype="multipart/form-data" novalidate style="display: grid; gap: 20px; max-width: 500px; margin-bottom: 30px;">
      ...
      <button type="submit" style="background: #0ea5e9; color: white; padding: 10px 20px; border: none; border-radius: 4px; cursor: pointer; font-weight: bold; justify-self: start;">Add Medicine</button>
    </form>
  </section> -->
